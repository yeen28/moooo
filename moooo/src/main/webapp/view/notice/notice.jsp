						...
						<c:forEach var="notice" items="${ selectedNotice }"> 
							<tr>
								...
								<td style="font-size: 16px; text-align: center; color: #333;">
								  <a href="<%= protocol %><%= domain %><%= contextRoot %>/view/notice/notice_detail.jsp?notice_id=${ notice.notice_id }">${ notice.title }</a>
								</td>
		...
		<nav id="pagination">
					<ul class="pagination">
						...
						<c:if test="${ nowPage ne 1 and param.page ne null }">
							<a href="<%= protocol %><%= domain %><%= contextRoot %>/view/notice/notice.jsp?page=${ nowPage-1 }" aria-label="Previous">
								<span aria-hidden="true">&laquo;</span>
							</a>
						</c:if>
						</li>
						<c:forEach var="i" begin="${ start }" end="${ end }">
						<li><a href="<%= protocol %><%= domain %><%= contextRoot %>/view/notice/notice.jsp?page=${ i }"><c:out value="${ i }"/></a></li>
						</c:forEach>
						<li>
						<% if(LastPage != 0 && nowPage != LastPage){ %>
							<a href="<%= protocol %><%= domain %><%= contextRoot %>/view/notice/notice.jsp?page=${ nowPage+1 }" aria-label="Next">
